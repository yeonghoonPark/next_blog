...
> 즉, 이미 저장된 값("apple")을 변경할 수 없기 때문에 `Call Stack`에 새로운 주소(0xB0000002)를 할당하고 값("banana")을 저장하는 것입니다. 그리고 이를 불변성이 유지되었다고 합니다.
...
> 즉, 변수 a와 b는 식별자로서 같은 주소(0xB0000002)를 가리키기 때문에 동일한 값이 됩니다.
...
1. `Call Stack`에 주소(0xC0000003)를 할당합니다.
2. `Momory Heap`에 주소(0x0000000A)를 할당하고 값(["a", "b"])을 저장합니다.
3. `Memory Heap`의 주소(0x0000000A)를 `Call Stack`에 주소(0xC0000003)에 대한 값으로 저장합니다.
4. 변수 alphabet은 식별자로서 `Call Stack`의 주소(0xC0000003)를 가리키게 됩니다.
...
1.  변수 alphabet이 식별자로서 `Call Stack`에서 가리키고 있는 주소(0xC0000003)를 찾습니다.
2.  해당 주소(0xC0000003)에 저장되어 있는 값(0x0000000A)을 기반으로 `Memory Heap`에서 주소(0x0000000A)를 찾습니다.
...
> 즉, `Call Stack`에 기존의 주소(0xC0000003)를 사용하면서 식별자가 최종적으로 가리키는 저장 된 값(["a", "b"])을 새로운 값(["a", "b", "c"])으로 변경합니다. 그리고 이를 불변성이 유지되지 않았다고 합니다.
...
1. `Call Stack`에 주소(0xD0000004)를 할당합니다.
2. `Momory Heap`에 주소(0x0000000B)를 할당하고 값(["a", "b", "c"])을 저장합니다.
3. `Memory Heap`의 주소(0x0000000B)를 `Call Stack`에 주소(0xD0000004)에 대한 값으로 저장합니다.
4. 변수 letters는 식별자로서 `Call Stack`의 주소(0xD0000004)를 가리키게 됩니다.
...
> 즉, 식별자로서 alphabet이 가리키는 주소(0xC0000003)와 letters가 가리키는 주소(0xD0000004)는 동일하지 않기 때문에 같은 값이 아닙니다.
