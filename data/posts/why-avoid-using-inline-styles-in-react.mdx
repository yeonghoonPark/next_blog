...
> object(객체) 타입인 props에 object(객체) 타입의 style을 정의하면 object(객체) 안에 object(객체)가 존재하는 nested object(중첩 객체)의 구조를 가지게 됩니다. 하지만 `Shallow Comparison(얕은 비교)`은 식별자가 가리키는 최상위 주소만을 참조합니다. 즉, React는 nested object(중첩 객체) 안에 있는 inner object(내부 객체)를 새로운 참조 값으로 인식하고 props가 변경되었다고 판단하기 때문에 re-rendering이 발생하게 됩니다.
