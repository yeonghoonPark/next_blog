...
`Post/Pre-Processors(전처리기)`나 `CSS-in-JS` library를 사용하면 inline style을 사용할 때 보다 `Readability(가독성)`를 올려주고 `Maintainability(유지보수)`또한 수월하게 만들어 줍니다.
...
inline style은 때에 따라 re-rendering을 할 때마다 새로운 object(객체)가 생성될 수 있습니다.  
...
1. 2번의 부모 컴포넌트에서 props로 정의해주는 style을 컴포넌트 외부로 이동시켜 caching 한다.
...
2. 만약 state에 따라 style이 변경되어야 한다면 컴포넌트 내부에서 `useMemo`를 이용해 caching 한다.
