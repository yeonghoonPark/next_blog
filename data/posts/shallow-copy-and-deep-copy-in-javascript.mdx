...
> ✳️ 복사는 단순한 값의 복제가 아닌 `원본과 관계없는 새로운 데이터를 생성`하는 것을 의미합니다.
...
> 얕은 복사와 깊은 복사 모두 원본과는 관계없는 독립적인 새로운 데이터를 생성하는 점은 동일합니다.  
...
> JavaScript에서 지원하는 위의 방법들은 1차원 형식의 array나 object에서는 복사가 이뤄집니다.  
> 하지만 array나 object 안에 array나 object가 위치하는 2차원 형식의 중첩 된 구조를 가지게 된다면 `1차원만 복사되고 2차원은 복사되지 않기 때문에 완전한 복사를 위해서는 Deep copy가 필요합니다.`
...
복사란 `원본을 손상하지 않는 동일한 데이터를 가진 새로운 데이터를 만드는 것`을 의미합니다.  
이를 `불변성을 유지한다`라고 말하며 불변성을 유지하므로 프로그래밍하는데 많은 이점을 가질 수 있습니다. 예측할 수 있는 코드의 흐름과 상태 값 추적을 통해 비교적 쉬운 디버깅이 가능하고 안정성과 유지보수성을 향상할 수 있습니다. `copy는 불변성을 유지하기 위한 방법`일 뿐이며 상황에 따라 적절한 방법을 사용하는 데 도움이 되길 바랍니다.
